...
def main
  ...
  begin
    if @options[:backend] == BACKEND_AG
      ...
      container_name = "#{@options[:backend]}-#{@options[:version]}-#{@options[:backend_port]}"
      run_cmd = "docker rm -f #{container_name} ; docker run -d --rm -p #{@options[:backend_port]}:#{@options[:backend_port]} --name #{container_name} bde2020/4store:#{@options[:version]}"
    ...
    redis_container_name = "redis-#{@options[:redis_port]}"
    run_cmd4 = "docker rm -f #{redis_container_name} ; docker run -p #{@options[:redis_port]}:6379  --name #{redis_container_name} -d redis"
    system("#{run_cmd4}")
