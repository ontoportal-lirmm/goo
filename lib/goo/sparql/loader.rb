  ...
  module SPARQL
    module Loader
      ...
      def self.model_load(*options)
        ...
        if options[:models] and options[:models].is_a?(Array) and\
          ...
          models.each_slice(Goo.slice_loading_size) do |model_slice|
            ...
            options[:include] = include_options.dup unless include_options.nil?
      ...
      def self.model_load_sliced(*options)
        ...
        select.options[:count] = [[:id, :count_var, :count]] if count
        if page
        ...
        if not graphs.nil?
          graphs.select! { |g| g.to_s["owl#Class"].nil? } if graphs.length > 0
        end
        ...
        objects_new = {}
        select.union_with_bind_as(*binding_as) if binding_as
        ...
        select.each_solution do |sol|
          ...
          variables.each do |v|
            ...
            object = object.object if object and !(object.kind_of? RDF::URI)
            ...
            if models_by_id[id].respond_to?(:klass)
              models_by_id[id][v] = object unless object.nil? && !models_by_id[id][v].nil?
            else
        ...
        if klass.collection_opts.instance_of?(Symbol)
          collection_value = collection.first if collection.is_a? Array and collection.length == 1
